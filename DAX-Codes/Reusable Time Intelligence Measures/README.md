...
```
MEASURE 'Measures Tables'[Amount Sold YTD] =
TOTALYTD([Amount Sold $], dates_table[Date Column])
...
MEASURE 'Measures Tables'[Amount Sold Past Quarter DATEADD] = 
CALCULATE(
    [Amount Sold $],
    DATEADD(dates_table[Date Column], -1, QUARTER)
)

MEASURE 'Measures Tables'[Amount Sold Past Month DATEADD] = 
CALCULATE(
    [Amount Sold $],
